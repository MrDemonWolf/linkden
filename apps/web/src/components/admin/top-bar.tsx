...
export function TopBar({
  ...
  return (
    <header className="h-[52px] flex items-center justify-between px-4 bg-[var(--admin-surface)] border-b border-[var(--admin-border)] shrink-0 relative z-10" style={{ boxShadow: "var(--admin-shadow-md)" }}>
      {/* Left: Logo */}
      <div className="flex items-center gap-4">
        <a href="/admin" className="flex items-center gap-0.5 group" aria-label="LinkDen Admin">
          <span className="text-base font-bold tracking-tight text-[var(--admin-accent)] transition-colors group-hover:text-[var(--admin-accent-hover)]" style={{ fontFamily: "var(--font-sora, 'Sora', sans-serif)" }}>Link</span>
          <span className="text-base font-bold tracking-tight text-[var(--admin-text)]" style={{ fontFamily: "var(--font-sora, 'Sora', sans-serif)" }}>Den</span>
        </a>
      </div>
      ...
      <div className="hidden md:flex items-center gap-2">
        <div className="flex items-center bg-[var(--admin-bg)] rounded-lg p-0.5 border border-[var(--admin-border-subtle)]">
          {deviceOptions.map(({ key, icon: Icon, title }) => (
            <button
              key={key}
              type="button"
        ...
        <div className="relative ml-1">
            ...
            <button
              ...
              style={{
                ...
                background: draftCount > 0
                  ? "linear-gradient(135deg, var(--admin-accent), var(--admin-accent-hover))"
                  : "var(--admin-accent)",
                boxShadow: draftCount > 0
                  ? "0 2px 12px rgba(99, 102, 241, 0.35)"
                  : undefined,
              }}
              aria-label={draftCount > 0 ? `Publish ${draftCount} changes` : "Publish"}
            >
              {isPublishing ? (
          ...
          {showPublishMenu && (
            <>
              ...
              <div className="absolute right-0 top-full mt-1.5 bg-[var(--admin-surface)] border border-[var(--admin-border)] rounded-lg py-1 min-w-[170px] z-50" style={{ boxShadow: "var(--admin-shadow-lg)" }} role="menu">
                <button
                  type="button"
                  role="menuitem"
                  onClick={() => {
