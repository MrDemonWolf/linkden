...
export const pagesRouter = router({
  ...
  create: protectedProcedure.input(CreatePageSchema).mutation(async ({ ctx, input }) => {
    ...
    const [existing] = await ctx.db.select().from(pages).where(eq(pages.slug, input.slug));
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const [deleted] = await ctx.db.delete(pages).where(eq(pages.id, input.id)).returning();
