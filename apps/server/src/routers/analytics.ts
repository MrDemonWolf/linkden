...
export const analyticsRouter = router({
  ...
  overview: protectedProcedure.input(AnalyticsQuerySchema).query(async ({ ctx, input }) => {
    ...
    const [views] = await ctx.db
      .select({ count: count() })
      .from(analytics)
      .where(and(eq(analytics.event, "page_view"), gte(analytics.createdAt, since)));

    const [clicks] = await ctx.db
      .select({ count: count() })
      .from(analytics)
      .where(and(eq(analytics.event, "link_click"), gte(analytics.createdAt, since)));
  ...
  timeSeries: protectedProcedure.input(AnalyticsQuerySchema).query(async ({ ctx, input }) => {
    const since = periodToDate(input.period);
  ...
  /** Clicks per link */
  byLink: protectedProcedure.input(AnalyticsQuerySchema).query(async ({ ctx, input }) => {
    const since = periodToDate(input.period);
  ...
  topReferrers: protectedProcedure.input(AnalyticsQuerySchema).query(async ({ ctx, input }) => {
    const since = periodToDate(input.period);
    ...
    const rows = await ctx.db
      ...
      .from(analytics)
      .where(and(gte(analytics.createdAt, since), sql`${analytics.referrer} != ''`))
  ...
  /** Country breakdown */
  countries: protectedProcedure.input(AnalyticsQuerySchema).query(async ({ ctx, input }) => {
    const since = periodToDate(input.period);
    ...
    const rows = await ctx.db
      ...
      .from(analytics)
      .where(and(gte(analytics.createdAt, since), sql`${analytics.country} != ''`))
