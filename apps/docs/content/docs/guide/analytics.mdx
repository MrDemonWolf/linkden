...
A bar chart powered by [Recharts](https://recharts.org/) showing daily page views for the selected period. The chart uses a `ChartContainer` wrapper with a `ResponsiveContainer` for automatic resizing. Bars highlight on hover and show the count via a tooltip. Date labels display month and day (e.g., "Feb 23").
