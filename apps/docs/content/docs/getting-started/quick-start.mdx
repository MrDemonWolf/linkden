...
The setup wizard is protected by a one-time security token. When your LinkDen server starts for the first time, it prints a setup URL to the **server console**:

```
Setup URL: http://localhost:3001/admin/setup?token=<unique-token>
```

Open this URL in your browser. If you navigate to `/admin/setup` without the token, you will see a "Setup Token Required" message directing you to check the server console.
...
- **Bio** -- A short tagline or description (up to 300 characters, optional).
...
A confirmation screen with a "Go to Builder" button that redirects to `/admin/builder`. The setup token is automatically invalidated once setup completes.
...
Navigate to `/admin/social` to manage social networks. The page features:

- **Combobox search** -- A searchable dropdown to quickly find networks by name.
- **Category filter tabs** -- Horizontal pill-style tabs (Social, Video, Music, Development, etc.) to browse by category.
- **Auto-invert icons** -- Brand icons that are too dark automatically invert in dark mode for visibility.

Toggle networks on/off and enter URLs. Active networks become available for use in **Social Icons** blocks.
