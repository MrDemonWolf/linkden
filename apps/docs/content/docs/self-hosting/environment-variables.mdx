...
| `NEXT_PUBLIC_SITE_URL` | Public URL of the site itself. Used for `robots.txt`, `sitemap.xml`, canonical links, and Open Graph URLs | `http://localhost:3001` |
...
| `ALCHEMY_STAGE` | Alchemy stage name. Defaults to `prod`; use another value (for example `dev`) for a second, independent deployment |
