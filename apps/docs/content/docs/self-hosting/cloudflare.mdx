...
2. Check the API Worker logs for the setup URL:
   ```bash
...
3. Open this URL in your browser. The token is required -- navigating to `/admin/setup` without it will show an error.
4. Complete the setup wizard to create your admin account (name, email, and password). The token is invalidated once setup completes.
5. After setup is complete, sign in at `/admin/login`.

### Set Auth Secrets on the API Worker
