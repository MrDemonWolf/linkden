...
1. Check the API service logs in Railway (click on the `linkden-api` service > **Deployments** > latest deployment > **Logs**) for the setup URL:
   ```
   Setup URL: https://yourdomain.com/admin/setup?token=<unique-token>
   ```
   The server generates a one-time secure token on first startup.
2. Open the setup URL in your browser. The token is required -- navigating to `/admin/setup` without it will show an error.
3. Complete the wizard to create your admin account (name, email, and password). The token is invalidated once setup completes.
4. After setup is complete, sign in at `/admin/login`.
5. Create a test link in the admin panel.
6. Visit the public page and verify the link appears.
7. Check analytics data is being recorded.
