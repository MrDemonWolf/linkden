...
After containers start, visit `http://localhost:3001/admin/setup` in your browser to create your admin account. The first user becomes the admin and registration locks automatically.
...
CMD ["bun", "run", "apps/server/src/node-entry.ts"]
```
...
```bash
cd apps/server && bun add @hono/node-server
cd ../../packages/db && bun add better-sqlite3 && bun add -d @types/better-sqlite3
```
...
RUN bun --filter @linkden/web build
...
1. Visit `https://yourdomain.com/admin/setup` in your browser.
2. Enter your name, email, and password to create your admin account.
3. The first user becomes the admin and registration locks automatically.
